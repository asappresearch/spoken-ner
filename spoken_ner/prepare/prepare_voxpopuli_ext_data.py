...
manifest_dir = "manifest/voxpopuli"
...
def generate_data_subset(manifest_dir, target_size=100):
	...
	save_dir = os.path.join(manifest_dir, "ext_data", "e2e_ner")

	for key in out_data:
		in_data[key] = utils.read_lst(os.path.join(manifest_dir, "ext_data", "e2e_ner", f"train_all_unlabeled.{key}"))
		if key == "tsv":
			out_data[key].append(in_data[key][0])

	permuted_idx_lst = random.shuffle(np.arange(len(in_data["wrd"])))
	...
	for key, data in out_data.items():
		utils.write_to_file("\n".join(data), os.path.join(save_dir, f"train_{target_size}h_unlabeled.{key}"))
		utils.write_to_file("\n".join(data), os.path.join(save_dir.replace("e2e_ner", "asr"), f"train_{target_size}h_unlabeled.{key}"))
...
def convert_plabeled_to_nlp_format(data_sfx="all"):
	...
	tagged_sent_lst = read_lst(os.path.join(SPEECH_NER_DATA_DIR, f"train_{data_sfx}_plabeled_nlp.wrd"))
	out_str = ""
	...
	for line in tagged_sent_lst:
		...
		for wrd in wrd_lst:
			if wrd in DATA_OBJ.spl_char_lst:
				if is_entity: # a new entity begun before completion of the previous entity
	...
	print("%d illegal assignments | %d sentences" % (num_illegal_assigments, len(tagged_sent_lst)))
	write_to_file(out_str, os.path.join(TEXT_NER_DATA_DIR, f"train_{data_sfx}_plabeled"))
...
def prep_unlabeled_data_files(voxpopuli_data_dir, slue_voxpopuli_dir, manifest_dir):
	...
	e2e_ner_save_dir = os.path.join(manifest_dir, "ext_data", "e2e_ner")
	os.makedirs(e2e_ner_save_dir, exist_ok=True)
	asr_save_dir = os.path.join(manifest_dir, "ext_data", "asr")
	os.makedirs(asr_save_dir, exist_ok=True)
	text_ner_save_dir = os.path.join(manifest_dir, "ext_data", "text_ner")
	...
	all_train_data_df = pd.read_csv(os.path.join(audio_data_dir, "asr_train.tsv"), sep="\t")
	tot_num_samples = len(all_train_data_df["id"].array)
	slue_finetune_data_df = pf.read_csv(os.path.join(slue_voxpopuli_dir, "slue-voxpopuli_fine-tune.tsv"))

	# write processed data
	with open(wrd_fn, "w") as f_wrd, open(ltr_fn, "w") as f_ltr, open(
		e2e_tsv_fn, "w") as f_e2e_tsv, open(text_ner_tsv_fn, "w"
		) as f_txt_ner_tsv:
		print(audio_data_dir, file=f_e2e_tsv)
		for data_sample in all_train_data_df.iterrows():
			utt_id = data_sample[1].id
			normalized_text = data_sample[1].normalized_text
			if utt_id not in slue_finetune_data_df["id"].array and normalized_text != "":
				entity_pair_str = data_utils.prep_text_ner_tsv(
                            normalized_text,
                            "None", # no labels
                            "combined", # value of this arg doesn't matter as this is unlabeled data
                        )
				print(entity_pair_str, file=f_txt_ner_tsv, end="")

                wrd_str, ltr_str = data_utils.prep_e2e_ner_files(
                    entity_pair_str, "combined"
                )
